...
from model import GCNModelScatTwoDecodersVAE, GCNModelStrAttScatTwoDecodersVAE, GCNModelDoubleAttScatTwoDecodersVAE, \
    GCNModelFeaAttScatTwoDecodersVAE
from utils import preprocess_graph, make_ad_dataset_both_anomaly, pred_anomaly, precision, dim_reduction, \
    make_ad_dataset_no_anomaly, make_ad_dataset_feature_anomaly, make_ad_dataset_structure_anomaly
...
parser.add_argument('--att', type=int, default=0, help='0 for no attention, 1 for all attention, 2 for feature attention, 3 for structure')
...
def gae_ad(args):
    ...
    att = ["NoAtt", "DoubleAtt", "FeaatureAtt", "StrAtt"]
    print()
    print()
    print()
    print("Initializing scat twodecoders training")
    print(args)
    ...
    y_features_name = "scat_output_twodecoders" + \
    ...
    reduced_y_features_name = "reduced_scat_output_twodecoders"+ \
                      "{}_".format(anomaly_type[args.data_type]) + \
                      "{}_".format(args.dataset) + \
    ...
    # Start training
    if args.att == 0:
        model = GCNModelScatTwoDecodersVAE(feat_dim, hidden1, hidden2, args.dropout).to(args.device)
    elif args.att == 1:
        model = GCNModelDoubleAttScatTwoDecodersVAE(feat_dim, hidden1, hidden2, args.dropout).to(args.device)
    elif args.att == 2:
        model = GCNModelFeaAttScatTwoDecodersVAE(feat_dim, hidden1, hidden2, args.dropout).to(args.device)
    elif args.att == 3:
        model = GCNModelStrAttScatTwoDecodersVAE(feat_dim, hidden1, hidden2, args.dropout).to(args.device)
    else:
