...
from model import GCNModelRsrScatTwoDecodersVAE
...
parser.add_argument('--att', type=int, default=0, help='0 for no attention, 1 for all attention,\
 2 for feature attention, 3 for structure. Note so far we only support no attention.')
...
def gae_ad(args):
    ...
    print()
    print()
    print("Initializing rsr scat twodecoders training")
    print(args)
    ...
    # Start training
    if args.att == 0:
        model = GCNModelRsrScatTwoDecodersVAE(feat_dim, hidden1, hidden2, args.dropout, rsr_dim).to(args.device)
    else:
    ...
    if torch.cuda.is_available() is True:
        ...
        for epoch in range(args.epochs):
            ...
            encoder_layer_2, rsrlayer, rsr_output = model(features, adj_norm)
    ...
    result_df.csv_path = 'rsr_scat_twodecoders' + \
